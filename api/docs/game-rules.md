...
- **Tree** — Blocks movement and blocks most shots. Notable exceptions: the **Chariot can
  shoot over trees**, and the **Necromancer's freeze is blocked only by trees**.
...
- **Jumping:** Jumps over any figures or obstacles in its path; only the **landing tile**
  ...
  empty tiles).
- **Attack:** Gamma-shaped (L) ranged attack up to **4 tiles**. It **shoots over trees**;
  all other obstacles and figures block the shot.
