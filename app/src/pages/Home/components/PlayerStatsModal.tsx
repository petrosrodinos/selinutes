...
export const PlayerStatsModal = ({ player, onClose }: PlayerStatsModalProps) => {
  ...
  const { progress, levelMeta } = getStatsData(points, level);
        ...
        {!isLoading && !isError && (
          <div className="space-y-4">
            <div className="flex items-center gap-4 p-4 bg-stone-900/60 rounded-xl border border-stone-700/40">
              <PlayerLevelBadge levelMeta={levelMeta} onClick={() => setPreviewLevel(levelMeta)} />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-stone-200">
                  Level {levelMeta.playerLevel} · {levelMeta.tierLabel}
                </p>
                <p className="text-xs text-stone-400 truncate">{levelMeta.figureTitle}</p>
                <div className="w-full h-1.5 bg-stone-700 rounded-full overflow-hidden mt-2">
                  <div
                ...
                {!atMaxLevel ? (
                  <p className="text-stone-500 text-xs mt-1">Progress to level {level + 1}</p>
                ) : null}
              </div>
